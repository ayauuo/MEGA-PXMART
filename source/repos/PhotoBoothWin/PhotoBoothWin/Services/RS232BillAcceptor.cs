    ...
    public class RS232BillAcceptor : IDisposable
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                // 嘗試找到可用的串口（COM3 保留給 Arduino 投幣器，紙鈔機不可使用）
                if (!TryFindPort(out string? foundPort))
                {
                    System.Diagnostics.Debug.WriteLine($"警告：找不到指定的串口 {_portName}，嘗試使用其他可用串口（排除 COM3）");
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("錯誤：沒有紙鈔機可用串口（僅 COM3 時保留給投幣器）");
                        return;
                    }
                }
        ...
        private bool TryFindPort(out string? portName)
        {
            ...
            foreach (string port in ports)
            {
                if (port.Equals(_portName, StringComparison.OrdinalIgnoreCase)
                    && !string.Equals(port, "COM3", StringComparison.OrdinalIgnoreCase))
                {
                    portName = port;
                    return true;
                }
            ...
            // 若指定埠不存在或為 COM3，使用第一個「非 COM3」的埠
