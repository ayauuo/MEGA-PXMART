...
namespace PhotoBoothWin
{
    public partial class MainWindow : Window
    {
            ...
            Loaded += async (_, __) =>
            {
                ...
                Web.CoreWebView2.DOMContentLoaded += (s, e) =>
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        Dispatcher.Invoke(() =>
                        {
                            // 先啟動投幣器（COM3），再啟動紙鈔機（避開 COM3），避免搶埠
                            StartCoinAcceptor(); // Arduino 投幣器 COM3，PULSES=數字 → 金額
                            StartBillAcceptor();
                        });
                    });
                };
        ...
        /// <summary>
        /// 啟動 Arduino 投幣器（COM3, 115200）。收到 PULSES=50 時會觸發 50 元付款事件。
        /// </summary>
        private void StartCoinAcceptor()
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    if (!ports.Any(p => string.Equals(p, "COM3", StringComparison.OrdinalIgnoreCase)))
                    {
                        System.Diagnostics.Debug.WriteLine("[投幣器] 未找到 COM3，跳過 Arduino 投幣器（請確認投幣器接在 COM3）");
                        return;
                    }
                    _coinAcceptor = new ArduinoCoinAcceptor("COM3", 115200);
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("✓ Arduino 投幣器已連接 (COM3)，等待 PULSES=數字 格式資料");
                        Dispatcher.Invoke(() => ShowStatusMessage("投幣器已連接 (COM3)", isError: false));
                    }
                    else
                    {
                        System.Diagnostics.Debug.WriteLine("[投幣器] COM3 開啟失敗（可能已被其他程式占用）");
                        NotifyCoinAcceptorPortInUse();
                    }
                }
                catch (UnauthorizedAccessException ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[投幣器] COM3 被占用或無權限：{ex.Message}");
                    NotifyCoinAcceptorPortInUse();
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// COM3 被占用時，在 UI 與 WebView 顯示解決方式。
        /// </summary>
        private void NotifyCoinAcceptorPortInUse()
        {
            const string msg = "COM3 被占用，請關閉 RS232-ICT004.exe 或其他使用 COM3 的程式";
            Dispatcher.Invoke(() =>
            {
        ...
        private string? AutoDetectBillAcceptorPort()
        {
            try
            {
                ...
                // 優先順序：COM7（紙鈔機專用），COM1、COM2、COM4…；COM3 保留給 Arduino 投幣器
                ...
                // COM3 保留給 Arduino 投幣器，紙鈔機不可使用
                var validComPorts = ports
                    .Where(p => p.StartsWith("COM", StringComparison.OrdinalIgnoreCase)
                        && !string.Equals(p, "COM3", StringComparison.OrdinalIgnoreCase))
                    .OrderBy(p => 
                    {
                        // 提取 COM 後的數字進行排序
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"找到有效串口（已排除 COM3）：{string.Join(", ", validComPorts)}");
                    System.Diagnostics.Debug.WriteLine($"使用第一個有效串口：{validComPorts[0]}");
                    return validComPorts[0];
                }
                
                // 若僅剩 COM3（已保留給投幣器），紙鈔機無可用串口
                System.Diagnostics.Debug.WriteLine("沒有紙鈔機可用串口（COM3 已保留給投幣器）");
                return null;
            }
            catch (Exception ex)
            {
